...
////                    Tuple3.of("a", 1L, 1000000079900L), //09:47:59
    ...
    public static class CountWithTimeoutFunction extends ProcessFunction<Tuple2<String, Long>, Tuple2<String, Long>> {
        ...
        public void processElement(Tuple2<String, Long> input, Context context, Collector<Tuple2<String, Long>> output) throws Exception {
            ...
            System.err.println("context.timestamp==>"+context.timestamp()+" "+current.key+" "+current.count);
//            System.out.println("元素"+input.f0+"进入事件时间为：" + new Date(current.lastModified));
            state.update(current);
//            System.out.println(new Date(current.lastModified));

            //注册ProcessTimer,更新一次就会有一个ProcessTimer
            context.timerService().registerEventTimeTimer(current.lastModified + 9000);

            System.out.println(context.timerService().currentWatermark()+"vvvvvvvvvvvvvvvv");
        }
        ...
        public void onTimer(long timestamp, OnTimerContext ctx, Collector<Tuple2<String, Long>> out) throws Exception {
            ...
            System.err.println("onTimer timestamp时间为："+new Date(timestamp)+"-----"+res.count);
//            System.err.println(ctx.timerService().currentWatermark()+"++++++++++++++++=");
//            System.out.println(timestamp+"-------"+res.lastModified);
...
//                            System.out.println("======>"+input.f2);
                ...
                }).keyBy(0).process(new CountWithTimeoutFunction()).print();
        env.execute();
